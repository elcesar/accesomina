...
test('all PostgreSQL migrations apply in order', async () => {
  ...
  const nexo=await db.query("SELECT id,status,is_domian_admin,admin_email,company_name FROM tenants WHERE regexp_replace(lower(rut),'[^0-9k]','','g')='784252132'");assert.equal(nexo.rows[0].status,'active');assert.equal(nexo.rows[0].is_domian_admin,true);assert.equal(nexo.rows[0].admin_email,'contacto@nexoklar.com');assert.equal(nexo.rows[0].company_name,'Nexo Klar SpA');
  await db.exec('BEGIN');await db.query("SELECT set_config('app.current_tenant_id',$1,true)",[nexo.rows[0].id]);const nexoUser=await db.query("SELECT role,active FROM app_users WHERE tenant_id=$1 AND lower(email)='contacto@nexoklar.com'",[nexo.rows[0].id]);assert.equal(nexoUser.rows[0].role,'domian_admin');assert.equal(nexoUser.rows[0].active,true);await db.exec('ROLLBACK');
