...
test('el menú React conserva el orden operacional oficial y separa la administración global', () => {
  for (const group of ['Centro de control', 'Capital humano', 'Gestión operacional', 'Contratistas', 'Relación comercial', 'Cumplimiento y calidad', 'Gestión de proyectos y negocios', 'Activos, equipos e inventario', 'Reportes y datos', 'Gestión y administración', 'Administración Nexo Klar']) {
    assert.ok(sidebar.includes(`['${group}',`), `grupo ausente: ${group}`);
  }
  const orderedIds = ['alertas', 'gestion-personal-proyecto', 'centro-operativo', 'personas', 'turnos-asistencia', 'proteccion-epp', 'formacion', 'examenes', 'salud-ocupacional', 'restringidos', 'comunicaciones', 'vehiculos', 'alojamientos', 'credenciales', 'incidentes', 'terceros-subcontratos', 'contratos-convenios', 'personal-empresa-servicios', 'habilitaciones-cumplimiento', 'evaluacion-desempeno', 'clientes', 'contratos', 'ordenes-servicio', 'cumplimiento-corporativo', 'habilitacion-cliente', 'auditoria', 'libro-obra', 'prospectos', 'activos-inventario', 'maquinaria', 'equipos-instrumentos', 'herramientas', 'epp-inventario', 'materiales', 'insumos', 'bodegas', 'movimientos-inventario', 'mantenimiento', 'asignaciones-prestamos', 'reportes', 'importar-exportar', 'configuracion', 'usuarios-permisos', 'bitacora', 'privacidad', 'administracion-clientes'];
  let previous = -1;
