...
function completeWorkflow(){return{
  ...
  firmas:[{id:'f1',trabId:'w2',mantId:'p1'}],libroObras:[{id:'lod1',folio:'MAE-2026-0001',book:'maestro',mantId:'p1',incidentId:'i1',signers:[{name:'Administrador Mandante',email:'mandante@example.com',status:'enviado'}],signatureEvents:[{id:'se1',status:'pending_config'}]}],callouts:[{id:'co1',mantId:'p1'}],waGroups:[{id:'g1',mantId:'p1',minaId:'m1',trabIds:['w1','w2']}],permisosTrabajo:[{id:'pt1',mantId:'p1'}]
};}
...
test('JSON export and import preserve the complete connected state',()=>{const exported=JSON.stringify({format:'accesomina-backup',version:1,modules:completeWorkflow()}),imported=JSON.parse(exported);const clean=validateTenantState(imported.modules);assert.equal(clean.trabajadores.length,2);assert.equal(clean.contratos[0].minaId,'m1');assert.equal(clean.firmas[0].mantId,'p1');assert.equal(clean.libroObras[0].incidentId,'i1');});
