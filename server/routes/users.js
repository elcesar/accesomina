...
usersRouter.patch('/:id', allowRoles('domian_admin','client_admin'), async (req, res) => {
  ...
  const row = await withTenant(req.auth.tenantId, async client => {
    ...
    if(before.rows[0].role==='domian_admin'&&((role&&role!=='domian_admin')||active===false))throw Object.assign(new Error('The Nexo Klar global administrator account is protected'),{status:409,code:'DOMIAN_ADMIN_PROTECTED'});
...
usersRouter.post('/:id/reset-password', allowRoles('domian_admin','client_admin'), async (req, res) => {
  ...
  const user=await withTenant(req.auth.tenantId,async client=>{
    ...
    if(before.role==='domian_admin'&&req.auth.role!=='domian_admin')throw Object.assign(new Error('The Nexo Klar global administrator account is protected'),{status:403,code:'DOMIAN_ADMIN_PROTECTED'});
