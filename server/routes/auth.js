...
const passwordResetLimiter = rateLimit({ windowMs: 15 * 60_000, limit: 5, standardHeaders: true, legacyHeaders: false, message:{message:'Espera unos minutos antes de solicitar otro enlace.'} });
...
authRouter.post('/forgot-password', passwordResetLimiter, async(req,res)=>{
  ...
  const email=normalizeEmail(req.body?.email),rutKey=normalizeRut(req.body?.rut);
  const generic={message:'Si los datos corresponden a una cuenta activa, recibirás un enlace para restablecer tu contraseña.'};
  const reply=async()=>{const wait=Math.max(0,300-(Date.now()-startedAt));if(wait)await new Promise(resolve=>setTimeout(resolve,wait));return res.json(generic);};
  ...
  const user=await withTenant(tenant.id,async client=>(await client.query('SELECT id,email,full_name,active FROM app_users WHERE tenant_id=$1 AND lower(email)=$2',[tenant.id,email])).rows[0]);
  if(!user?.active)return reply();
  const request=await createPasswordReset({tenantId:tenant.id,user,ip:clientIp(req)});
  void deliverPasswordReset({email:user.email,fullName:user.full_name,url:request.url}).catch(()=>{});
  return reply();
});

authRouter.post('/reset-password', passwordResetLimiter, async(req,res)=>{
  const token=String(req.body?.token||''),tenantId=String(req.body?.tenant||''),newPassword=String(req.body?.newPassword||''),confirmation=String(req.body?.confirmation||'');
  if(!/^[0-9a-f-]{36}$/i.test(tenantId)||!token||newPassword!==confirmation||!validatePassword(newPassword))return res.status(400).json({error:'INVALID_RESET_REQUEST',message:'El enlace o la nueva contraseña no son válidos.'});
  const reset=await withTenant(tenantId,async client=>(await client.query(`SELECT r.id,r.user_id,u.email,u.full_name FROM password_reset_tokens r JOIN app_users u ON u.id=r.user_id AND u.tenant_id=r.tenant_id WHERE r.tenant_id=$1 AND r.token_hash=$2 AND r.used_at IS NULL AND r.expires_at>now() AND u.active LIMIT 1`,[tenantId,sha256(token)])).rows[0]);
  ...
  const changed=await withTenant(tenantId,async client=>{
    const used=(await client.query('UPDATE password_reset_tokens SET used_at=now() WHERE id=$1 AND tenant_id=$2 AND used_at IS NULL AND expires_at>now() RETURNING id',[reset.id,tenantId])).rows[0];
    if(!used)return false;
    await client.query('UPDATE app_users SET password_hash=$1,password_salt=$2,must_change_password=false,failed_login_count=0,locked_until=NULL,updated_at=now() WHERE id=$3 AND tenant_id=$4',[credentials.hash,credentials.salt,reset.user_id,tenantId]);
    await client.query('UPDATE user_sessions SET revoked_at=now() WHERE user_id=$1 AND tenant_id=$2 AND revoked_at IS NULL',[reset.user_id,tenantId]);
    await appendAudit(client,{tenantId,userId:reset.user_id,entityType:'user',entityId:reset.user_id,action:'user.password_reset_completed',newValue:{sessionsRevoked:true}});
    return true;
  });
