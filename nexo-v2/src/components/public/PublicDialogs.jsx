...
export function DemoRequestDialog({ onClose }) {
  ...
  const submit = event => { event.preventDefault(); const body=Object.entries(form).map(([key,value])=>`${key}: ${value}`).join('\n'); window.location.href=`mailto:contacto@nexoklar.com?subject=${encodeURIComponent('Solicitud de demostración Nexo Klar')}&body=${encodeURIComponent(body)}`; onClose() }
...
export function InformationDialog({ kind, onClose }) {
  const content = kind==='faq' ? { eyebrow:'Preguntas frecuentes', title:'Lo esencial antes de implementar.', blocks:[['¿Para qué empresas sirve Nexo Klar?','Para empresas de servicios que necesitan relacionar personas, contratos, órdenes de servicio, documentos y recursos en una misma operación.'],['¿Podemos cargar información histórica?','Sí. La carga puede ser gradual, individual o masiva, priorizando los procesos y la información que cada empresa necesita controlar.'],['¿Las empresas comparten información?','No. Cada empresa trabaja en un espacio privado con sus propios usuarios, permisos, configuraciones e información.'],['¿Qué acompañamiento recibe el equipo?','La puesta en marcha considera configuración inicial, carga priorizada y acompañamiento según el alcance contratado.']] } : { eyebrow:'Información comercial', title:'Términos y privacidad.', blocks:[['Espacios privados por empresa','Nexo Klar opera con usuarios, permisos, configuraciones e información separada por empresa.'],['Condiciones de servicio','El alcance, responsabilidades y tratamiento de datos personales se formalizan en la propuesta y documentación contractual vigente para cada cliente.']] }
