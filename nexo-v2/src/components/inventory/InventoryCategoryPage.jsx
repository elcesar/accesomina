...
export default function InventoryCategoryPage({ category, title, description, singular, defaultType, focus = 'stock' }) {
  ...
  const filtered = useMemo(() => items.filter(item => {
    ...
    const stateLabel = focus === 'machinery' ? machineryState(item, activeAssignments) : focus === 'equipment' ? equipmentState(item) : stockState(item)
  ...
  const summary = useMemo(() => {
    ...
    const custodians = focus === 'equipment' ? items.filter(item => String(item.custodian || '').trim()).length : 0
    return { total: items.length, low, assigned: delivered, due, available, calibrated, certified, custodians }
  ...
  const lastColumnLabel = focus === 'tools' ? 'Asignación / devolución' : focus === 'epp' ? 'Talla / vida útil' : 'Mantenimiento / estado'
