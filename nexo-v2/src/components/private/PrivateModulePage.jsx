...
import { CANONICAL_STATUSES, isOther } from '../../config/dataVocabulary.js'
...
function CreateRecordDialog({ module, state, versions, onClose, onSaved }) {
  ...
  const initialForm = { nombre: '', descripcion: '', estado: 'activo', responsable: '', vinculo: '', minaId: '', contratoId: '', mantId: '', ...Object.fromEntries(experience.formFields.map(field => [field.key, ''])) }
  const [form, setForm] = useState(initialForm)
  const [file, setFile] = useState(null)
  const [step, setStep] = useState(1)
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const clients = asRows(state?.minas?.length ? state.minas : state?.clientes)
  const contracts = asRows(state?.contratos)
  const orders = asRows(state?.mantenciones?.length ? state.mantenciones : state?.proyectos)
  const contractsForClient = form.minaId ? contracts.filter(item => String(item.minaId) === String(form.minaId)) : []
  const ordersForContract = form.contratoId ? orders.filter(item => String(item.contratoId) === String(form.contratoId)) : []
  const fields = {
  ...
  const save = async event => {
    ...
    try {
      ...
      const detailFields = Object.fromEntries(experience.formFields.flatMap(field => {
        const value = String(form[field.key] || '').trim()
        const detail = String(form[`${field.key}Detalle`] || '').trim()
        return [[field.key, value], ...(isOther(value) && detail ? [[`${field.key}Detalle`, detail]] : [])]
      }).filter(([, value]) => value))
      const item = { id, nombre: form.nombre.trim(), descripcion: form.descripcion.trim(), estado: form.estado, responsable: form.responsable.trim(), vinculo: form.vinculo.trim(), minaId: form.minaId || undefined, contratoId: form.contratoId || undefined, mantId: form.mantId || undefined, ...detailFields, createdAt: new Date().toISOString(), ...(uploaded ? { fileId: uploaded.id, archivo: uploaded.original_name } : {}) }
  ...
  return <div className="nk-dialog-backdrop" role="presentation" onMouseDown={onClose}>
    <form className="nk-dialog" role="dialog" aria-modal="true" aria-labelledby={`dialog-${module.id}`} onKeyDown={closeWithEscape} onSubmit={save} onMouseDown={event => event.stopPropagation()}>
      ...
      {step === 1 ? <><label>{fields.name}<input autoFocus required value={form.nombre} onChange={event => setForm({ ...form, nombre: event.target.value })} placeholder={fields.example} /></label><label>Detalle inicial<textarea value={form.descripcion} onChange={event => setForm({ ...form, descripcion: event.target.value })} placeholder={fields.detail} rows="4" /></label>{experience.formFields.length > 0 && <div className="nk-dialog-fields">{experience.formFields.map(field => <label key={field.key}>{field.label}{field.type === 'select' ? <><select value={form[field.key]} onChange={event => setForm({ ...form, [field.key]: event.target.value, [`${field.key}Detalle`]: event.target.value === 'otro' ? form[`${field.key}Detalle`] || '' : '' })}>{field.options.map(item => <option key={item.value} value={item.value}>{item.label}</option>)}</select>{isOther(form[field.key]) && <input value={form[`${field.key}Detalle`] || ''} onChange={event => setForm({ ...form, [`${field.key}Detalle`]: event.target.value })} placeholder={`Describe ${field.label.toLocaleLowerCase('es-CL')}`} />}</> : <input type={field.type || 'text'} value={form[field.key]} onChange={event => setForm({ ...form, [field.key]: event.target.value })} placeholder={field.placeholder} />}</label>)}</div>}</> : <><div className="nk-dialog-fields"><label>Responsable<input value={form.responsable} onChange={event => setForm({ ...form, responsable: event.target.value })} placeholder="Persona o equipo responsable" /></label><label>Cliente<select value={form.minaId} onChange={event => setForm({ ...form, minaId: event.target.value, contratoId: '', mantId: '' })}><option value="">Sin cliente asociado</option>{clients.map(client => <option key={client.id} value={client.id}>{client.nombre}</option>)}</select></label><label>Contrato<select disabled={!form.minaId} value={form.contratoId} onChange={event => setForm({ ...form, contratoId: event.target.value, mantId: '' })}><option value="">Sin contrato asociado</option>{contractsForClient.map(contract => <option key={contract.id} value={contract.id}>{contract.numero || contract.nombre}</option>)}</select></label><label>Orden de servicio<select disabled={!form.contratoId} value={form.mantId} onChange={event => setForm({ ...form, mantId: event.target.value })}><option value="">Sin orden asociada</option>{ordersForContract.map(order => <option key={order.id} value={order.id}>{order.codigo || order.nombre}</option>)}</select></label></div><label>Estado<select value={form.estado} onChange={event => setForm({ ...form, estado: event.target.value })}>{CANONICAL_STATUSES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="nk-file-field"><IconPaperclip size={16}/><span>{file?.name || 'Adjuntar evidencia (opcional)'}</span><input type="file" accept="application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event => setFile(event.target.files?.[0] || null)} /></label><p className="nk-dialog-copy">Podrás complementar o corregir esta información más adelante sin perder el historial.</p></>}
