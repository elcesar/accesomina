...
function CreateRecordDialog({ module, state, versions, onClose, onSaved }) {
  ...
  return <div className="nk-dialog-backdrop" role="presentation" onMouseDown={onClose}>
    <form className="nk-dialog" onSubmit={save} onMouseDown={event => event.stopPropagation()}>
      ...
      {step === 1 ? <><label>Nombre o identificador<input autoFocus required value={form.nombre} onChange={event => setForm({ ...form, nombre: event.target.value })} placeholder={`Ej.: ${module.title}`} /></label><label>Descripción<textarea value={form.descripcion} onChange={event => setForm({ ...form, descripcion: event.target.value })} placeholder="Agrega solo el detalle que ayudará al equipo a gestionarlo." rows="4" /></label></> : <><div className="nk-dialog-fields"><label>Responsable<input value={form.responsable} onChange={event => setForm({ ...form, responsable: event.target.value })} placeholder="Persona o equipo responsable" /></label><label>Relacionado con<input value={form.vinculo} onChange={event => setForm({ ...form, vinculo: event.target.value })} placeholder="Cliente, contrato, orden o recurso" /></label></div><label>Estado<select value={form.estado} onChange={event => setForm({ ...form, estado: event.target.value })}><option value="activo">Activo</option><option value="en_revision">En revisión</option><option value="pendiente">Pendiente</option></select></label><label className="nk-file-field"><IconPaperclip size={16}/><span>{file?.name || 'Adjuntar evidencia (opcional)'}</span><input type="file" accept="application/pdf,image/jpeg,image/png,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event => setFile(event.target.files?.[0] || null)} /></label><p className="nk-dialog-copy">Podrás complementar o corregir esta información más adelante sin perder el historial.</p></>}
