  ...
  return (
    <div className="nk-public-site">
      ...
      <footer className="nk-public-footer">
        ...
        <div>
          <button onClick={() => setDialog('faq')}>Preguntas frecuentes</button>
          <button onClick={() => setDialog('legal')}>Términos y privacidad</button>
          <a href="mailto:contacto@nexoklar.cl">contacto@nexoklar.cl</a>
        </div>
      </footer>
      {preview && (
        <div className="nk-lightbox" role="dialog" aria-modal="true" onClick={() => setPreview(false)}>
          <button aria-label="Cerrar vista ampliada" onClick={() => setPreview(false)}>×</button>
          <img src="/dashboard-demo.png" alt="Vista ampliada de Nexo Klar" onClick={e => e.stopPropagation()} />
        </div>
      )}
