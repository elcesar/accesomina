...
function PersonCell({ name, rut, onClick }) {
  const content = <><span className="nk-epp-person-avatar" aria-hidden="true">{initials(name)}</span><span className="nk-epp-person-copy"><span className="nk-epp-person-name">{name || 'Sin nombre'}</span><span className="nk-epp-person-rut">{rut || 'Sin RUT'}</span></span></>
  return onClick ? <button className="nk-epp-person" type="button" onClick={onClick} aria-label={`Abrir ficha de ${name || 'persona'}`}>{content}</button> : <div className="nk-epp-person">{content}</div>
}
...
export default function ProteccionEppPage() {
  ...
  return <div className="nk-epp-page">
    ...
    {view==='personas'&&<section className="nk-card nk-epp-table-card">{loading?<div className="nk-empty nk-epp-empty"><IconUsers size={30}/><p className="nk-empty-title">Cargando personas…</p></div>:!personRows.length?<div className="nk-empty nk-epp-empty"><p className="nk-empty-title">Sin personas para mostrar</p></div>:<div className="nk-table-wrapper"><table className="nk-table"><thead><tr><th>Persona</th><th>Función</th><th>Entregas</th><th>Pendientes / reposición</th><th>Tallas registradas</th></tr></thead><tbody>{personRows.map(({worker,own,pending,sizes})=><tr key={worker.id}><td><PersonCell name={worker.nombre} rut={worker.rut} onClick={()=>navigate(`/app/trabajadores/${worker.id}`)} /></td><td>{worker.cargo||worker.especialidad||'—'}</td><td>{own.length}</td><td><span className={`nk-badge ${pending.length?'nk-badge-warn':'nk-badge-ok'}`}>{pending.length?`${pending.length} por revisar`:'Al día'}</span></td><td>{sizes||'Sin tallas'}</td></tr>)}</tbody></table></div>}</section>}
    ...
    {view==='historial'&&<section className="nk-card nk-epp-table-card">{loading?<div className="nk-empty nk-epp-empty"><IconShield size={30}/><p className="nk-empty-title">Cargando entregas…</p></div>:filtered.length===0?<div className="nk-empty nk-epp-empty"><IconPackage size={30}/><p className="nk-empty-title">Sin entregas para mostrar</p></div>:<div className="nk-table-wrapper"><table className="nk-table"><thead><tr><th>Persona</th><th>EPP</th><th>Cantidad</th><th>Bodega / orden</th><th>Entrega</th><th>Reposición</th><th>Estado</th></tr></thead><tbody>{filtered.map((item,index)=>{const status=replaceStatus(item.replaceAt);const warehouse=warehouses.find(entry=>String(entry.id)===String(item.warehouseId));const order=orders.find(entry=>String(entry.id)===String(item.orderId));return<tr key={item.id||index}><td><PersonCell name={item.workerName} rut={item.workerRut} onClick={item.workerId?()=>navigate(`/app/trabajadores/${item.workerId}`):undefined} /></td><td><div className="nk-epp-item"><strong>{item.itemName}</strong><span className="nk-epp-subtle">{item.brandModel||'Sin marca/modelo'}{item.certification?` · ${item.certification}`:''}</span></div></td><td>{item.quantity || 1}{item.size ? ` · ${item.size}` : ''}</td><td><span className="nk-epp-subtle">{warehouse?.nombre || warehouse?.name || 'Sin bodega'}{order ? ` · ${order.codigo || order.nombre || order.name}` : ''}</span></td><td>{item.deliveredAt||'—'}</td><td>{item.replaceAt||'Según inspección'}</td><td><span className={`nk-badge ${status.cls}`}>{status.label}</span></td></tr>})}</tbody></table></div>}</section>}
