...
import { IconArrowLeft, IconCheck, IconId, IconPlus, IconRefresh, IconSearch } from '@tabler/icons-react'
...
const stateLabel=value=>({vigente:'Vigente',proximo:'Por vencer',vencido:'Vencida',falta:'Sin vencimiento',no_habilitada:'No habilitada'}[value]||value)
...
const credentialState=(item,worker)=>isRestricted(worker)?'no_habilitada':dateState(item?.vence)
...
export default function CredencialesPage(){
 ...
 const noHabilitadas=credentials.filter(x=>['vencido','falta','no_habilitada'].includes(credentialState(x,workerById.get(String(x.trabId))))).length
 ...
 async function save(event){
  ...
  try{
   ...
   const saved={...draft,id,numero:draft.numero.trim(),zona:draft.zona.trim(),observacion:draft.observacion.trim(),estado:credentialState(draft,credentialWorker),evidenceFiles:rows(draft.evidenceFiles),updatedAt:now,...(creating?{createdAt:draft.createdAt||now}:{})}
 ...
 return <section className="nk-credentials-page">
  ...
  <section className="nk-card nk-credentials-table-card"><div className="nk-credentials-section-head"><div><strong>Credenciales registradas</strong><small>{visible.length} de {credentials.length} registros</small></div></div><div className="nk-table-wrapper"><table className="nk-table nk-credentials-table"><thead><tr><th>Persona</th><th>Cliente / faena</th><th>Pase / zona</th><th>Emisión</th><th>Vencimiento</th><th>Campamento</th><th>Estado</th><th>Respaldo</th><th/></tr></thead><tbody>{loading?<tr><td colSpan="9">Cargando…</td></tr>:!visible.length?<tr><td colSpan="9">Sin credenciales para mostrar.</td></tr>:visible.map(item=>{const worker=workerById.get(String(item.trabId));const client=clientById.get(String(item.minaId));const currentState=credentialState(item,worker);return <tr key={item.id}><td>{worker?<Link className="nk-credentials-entity-link" to={`/app/trabajadores/${encodeURIComponent(worker.id)}`} aria-label={`Abrir ficha de ${worker.nombre||worker.rut||worker.id}`}><span className="nk-credentials-person-avatar" aria-hidden="true">{String(worker.nombre||worker.rut||'?').trim().split(/\s+/).slice(0,2).map(part=>part[0]).join('').toUpperCase()}</span><span className="nk-credentials-person-copy"><strong>{worker.nombre||worker.id}</strong><small>{worker.rut||'Sin RUT'}</small></span></Link>:item.trabId||'—'}</td><td>{client?<Link className="nk-credentials-entity-link" to={`/app/clientes/${encodeURIComponent(client.id)}`}>{client.nombre||client.id}</Link>:item.minaId||'—'}</td><td><button className="nk-credentials-entity-link nk-credentials-credential-link" type="button" onClick={()=>openItem(item)} aria-label={`Abrir credencial ${item.numero||item.tipo||''}`}><span><strong>{item.tipo||'Pase'} · {item.numero||'—'}</strong><small>{item.zona||'Sin zona definida'}</small></span></button></td><td>{item.emision||'—'}</td><td>{item.vence||'—'}</td><td>{item.campamento==='si'?'Sí':'No'}</td><td><span className={`nk-credential-state ${currentState}`}>{stateLabel(currentState)}</span></td><td>{rows(item.evidenceFiles).length?<span className="nk-badge nk-badge-none">{rows(item.evidenceFiles).length} archivo(s)</span>:<span className="nk-credentials-muted">Sin respaldo</span>}</td><td/></tr>})}</tbody></table></div></section>
  {(creating||selected)&&<form className="nk-card nk-credentials-workspace" onSubmit={save}><div className="nk-credentials-workspace-head"><div><button className="nk-credentials-back" type="button" onClick={closeDetail}><IconArrowLeft size={16}/>Volver a credenciales</button><p className="nk-credentials-kicker">{creating?'Nueva credencial':'Ficha de credencial'}</p><h2>{creating?'Registrar credencial':`${selectedWorker?.nombre||'Persona'} · ${draft.numero||draft.tipo||'Credencial'}`}</h2></div><div className="nk-actions">{canEdit&&<button className="nk-button nk-button-primary" type="submit" disabled={saving}><IconCheck size={16}/>{saving?'Guardando…':'Guardar cambios'}</button>}</div></div><div className="nk-credentials-form-grid"><label className="nk-field nk-credentials-span-2"><span className="nk-label">Persona</span><select className="nk-select" disabled={!canEdit} value={draft.trabId} onChange={update('trabId')}><option value="">Seleccionar persona</option>{workers.map(w=><option key={w.id} value={w.id}>{w.nombre||w.rut||w.id}</option>)}</select></label><label className="nk-field nk-credentials-span-2"><span className="nk-label">Cliente / faena</span><select className="nk-select" disabled={!canEdit} value={draft.minaId} onChange={update('minaId')}><option value="">Seleccionar cliente</option>{clients.map(c=><option key={c.id} value={c.id}>{c.nombre||c.mandante||c.id}</option>)}</select></label><label className="nk-field"><span className="nk-label">Tipo</span><select className="nk-select" disabled={!canEdit} value={draft.tipo} onChange={update('tipo')}><option value="fisico">Físico</option><option value="digital">Digital</option><option value="QR">QR</option><option value="temporal">Temporal</option></select></label><label className="nk-field"><span className="nk-label">Número / QR</span><input className="nk-input" disabled={!canEdit} value={draft.numero} onChange={update('numero')}/></label><label className="nk-field"><span className="nk-label">Emisión</span><input className="nk-input" type="date" disabled={!canEdit} value={draft.emision} onChange={update('emision')}/></label><label className="nk-field"><span className="nk-label">Vencimiento</span><input className="nk-input" type="date" disabled={!canEdit} value={draft.vence} onChange={update('vence')}/></label><label className="nk-field nk-credentials-span-2"><span className="nk-label">Zona autorizada</span><input className="nk-input" disabled={!canEdit} value={draft.zona} onChange={update('zona')} placeholder="Portería, planta, casino..."/></label><label className="nk-field"><span className="nk-label">Acceso campamento</span><select className="nk-select" disabled={!canEdit} value={draft.campamento} onChange={update('campamento')}><option value="no">No</option><option value="si">Sí</option></select></label><label className="nk-field"><span className="nk-label">Estado calculado</span><div className="nk-credentials-readonly">{stateLabel(dateState(draft.vence))}</div></label><label className="nk-field nk-credentials-wide"><span className="nk-label">Observación</span><textarea className="nk-textarea" rows="3" disabled={!canEdit} value={draft.observacion} onChange={update('observacion')}/></label></div><section className="nk-credentials-context"><strong>Contexto operacional</strong><span>Persona: {selectedWorker?<Link className="nk-credentials-entity-link" to={`/app/trabajadores/${encodeURIComponent(selectedWorker.id)}`}>{selectedWorker.nombre||selectedWorker.id}</Link>:'Sin selección'}</span><span>Cliente/faena: {selectedClient?<Link className="nk-credentials-entity-link" to={`/app/clientes/${encodeURIComponent(selectedClient.id)}`}>{selectedClient.nombre||selectedClient.id}</Link>:'Sin selección'}</span><span>Respaldo actual: {rows(draft.evidenceFiles).length} archivo(s)</span></section></form>}
 </section>
}
