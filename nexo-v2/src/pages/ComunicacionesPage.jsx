...
export default function ComunicacionesPage({createMode=false}){
 ...
 useEffect(()=>{
   ...
   const current=callouts.find(x=>String(x.id)===String(selectedId))
   if(!current){setSelectedId(null);setDraft(emptyDraft());return}
   setDraft({...emptyDraft(),...current,titulo:current.titulo||current.nombre||current.title||'',mensaje:current.mensaje||current.descripcion||'',especialidades:rows(current.especialidades),recipients:rows(current.recipients)})
 },[callouts,selectedId,creating])
