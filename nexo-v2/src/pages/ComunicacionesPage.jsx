...
export default function ComunicacionesPage(){
 ...
 const pendingCount=callouts.filter(x=>!['confirmado','completado','cerrado','cancelado'].includes(normalize(x.estado||x.status||'pendiente'))).length
 const sentTotal=callouts.reduce((sum,item)=>sum+Number(item.enviados||0),0)
 const repliedTotal=callouts.reduce((sum,item)=>sum+Number(item.respondieron||0),0)
 const assignedTotal=callouts.reduce((sum,item)=>sum+Number(item.asignados||0),0)
 ...
 return <section className="nk-comms-page">
  ...
  <section className="nk-comms-summary"><article><IconMessageCircle size={18}/><b>{callouts.length}</b><span>Comunicaciones</span></article><article><IconSend size={18}/><b>{sentTotal}</b><span>Enviados</span></article><article><IconUsers size={18}/><b>{repliedTotal}</b><span>Respondieron</span></article><article><IconCheck size={18}/><b>{assignedTotal}</b><span>Asignados</span></article></section>
