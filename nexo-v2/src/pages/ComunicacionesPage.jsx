...
export default function ComunicacionesPage(){
 ...
 return <section className="nk-comms-page">
  ...
  <section className="nk-card nk-comms-register">
   ...
   {loading?<div className="nk-comms-empty">Cargando…</div>:!visible.length?<div className="nk-comms-empty">No hay comunicaciones para mostrar.</div>:<div className="nk-table-wrapper"><table className="nk-table nk-comms-table"><thead><tr><th>Comunicación / convocatoria</th><th>Orden de servicio</th><th>Especialidad / Persona</th><th>Turno</th><th>Cupos</th><th>Enviados</th><th>Respondieron</th><th>Asignados</th><th>Estado</th><th></th></tr></thead><tbody>{visible.map(item=>{const order=orderById.get(String(item.mantId));const person=workerById.get(String(item.trabId||item.trabajadorId||item.personaId));return <tr key={item.id}><td><strong>{labelFor(item)}</strong><small>{item.fecha||item.responderAntes||'Sin fecha'} · {item.tipo||'comunicación'}</small></td><td>{order?<Link className="nk-context-link" to={`/app/servicios/${encodeURIComponent(order.id)}`}>{order.codigo||order.nombre||order.id}</Link>:'—'}</td><td>{rows(item.especialidades).length?rows(item.especialidades).join(', '):person?<Link className="nk-context-link" to={`/app/trabajadores/${encodeURIComponent(person.id)}`}>{person.nombre||person.id}</Link>:'—'}</td><td>{item.turno||'—'}</td><td>{Number(item.cupos||0)||'—'}</td><td>{Number(item.enviados||0)}</td><td>{Number(item.respondieron||0)}</td><td>{Number(item.asignados||0)}</td><td><span className="nk-badge nk-badge-none">{statusLabel(item.estado||item.status)}</span></td><td><button className="nk-button nk-button-quiet" type="button" onClick={()=>selectItem(item.id)}>Abrir ficha</button></td></tr>})}</tbody></table></div>}
  </section>
