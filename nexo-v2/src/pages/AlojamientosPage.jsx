  ...
  return <section className="nk-lodging-page">
      ...
      <div className="nk-lodging-cards">
        {loading ? <div className="nk-card nk-lodging-empty">Cargando…</div> : !visibleHotels.length ? <div className="nk-card nk-lodging-empty">No hay alojamientos para mostrar.</div> : visibleHotels.map(hotel => {
          ...
          return <article className="nk-card nk-lodging-hotel-card" key={hotel.id}>
            <button type="button" className="nk-lodging-entity-link nk-lodging-hotel-link" onClick={() => openHotel(hotel)} aria-label={`Abrir ficha de ${hotel.nombre}`}><IconBed size={18}/><div><strong>{hotel.nombre}</strong><span>{hotel.direccion || hotel.ciudad || 'Sin dirección'}</span></div></button>
            ...
            <div className="nk-lodging-hotel-foot"><small>{occupied}/{beds || 0} camas ocupadas</small></div>
          </article>
        })}
      </div>
    </section>
    ...
    {showHotelForm && <form className="nk-card nk-lodging-panel" onSubmit={saveHotel}>
      ...
      <div className="nk-lodging-form-grid">
        ...
        <label className="nk-field nk-lodging-span-2"><span className="nk-label">Dirección</span><input className="nk-input" disabled={!canManageHotels} value={hotelDraft.direccion} onChange={updateHotel('direccion')}/></label>
    ...
    {showStayForm && <form className="nk-card nk-lodging-panel" onSubmit={saveStay}>
      ...
      <div className="nk-lodging-panel-actions">{selectedWorker && <span>Persona: <Link className="nk-lodging-entity-link" to={`/app/trabajadores/${encodeURIComponent(selectedWorker.id)}`}>{selectedWorker.nombre}</Link></span>}{selectedOrder && <span>OS: <Link className="nk-lodging-entity-link" to={`/app/servicios/${encodeURIComponent(selectedOrder.id)}`}>{selectedOrder.codigo || selectedOrder.nombre}</Link></span>}{selectedClient && <span>Cliente: <Link className="nk-lodging-entity-link" to={`/app/clientes/${encodeURIComponent(selectedClient.id)}`}>{selectedClient.nombre}</Link></span>}<button className="nk-button nk-button-primary" type="submit" disabled={saving}><IconPlus size={16}/>{saving ? 'Guardando…' : 'Asignar estadía'}</button></div>
    </form>}
    ...
    <section className="nk-card nk-lodging-stays">
      ...
      {!stays.length ? <div className="nk-lodging-empty">Sin alojamientos asignados.</div> : <div className="nk-table-wrapper"><table className="nk-table"><thead><tr><th>Persona</th><th>Cliente</th><th>Orden de servicio</th><th>Alojamiento / habitación</th><th>Estadía</th><th>Turno</th><th>Estado</th></tr></thead><tbody>{stays.slice().sort((a, b) => String(b.checkin || '').localeCompare(String(a.checkin || ''))).map(stay => {
        ...
        return <tr key={stay.id}><td>{worker ? <Link className="nk-lodging-entity-link" to={`/app/trabajadores/${encodeURIComponent(worker.id)}`}>{worker.nombre || worker.id}</Link> : stay.trabId}</td><td>{client ? <Link className="nk-lodging-entity-link" to={`/app/clientes/${encodeURIComponent(client.id)}`}>{client.nombre || client.id}</Link> : '—'}</td><td>{order ? <Link className="nk-lodging-entity-link" to={`/app/servicios/${encodeURIComponent(order.id)}`}>{order.codigo || order.nombre || order.id}</Link> : stay.mantId}</td><td>{hotel ? <button type="button" className="nk-lodging-entity-link" onClick={() => openHotel(hotel)}>{hotel.nombre || hotel.id}{stay.pieza ? ` · #${stay.pieza}` : ''}</button> : <>{stay.hotelId}{stay.pieza ? ` · #${stay.pieza}` : ''}</>}</td><td>{stay.checkin || '—'} → {stay.checkout || '—'}</td><td>{stay.turno || '—'}</td><td><span className={`nk-badge ${stateLabel === 'ocupada' ? 'nk-badge-ok' : stateLabel === 'futura' ? 'nk-badge-info' : 'nk-badge-none'}`}>{stateLabel === 'ocupada' ? 'Ocupada' : stateLabel === 'futura' ? 'Futura' : statusLabel(stateLabel)}</span></td></tr>
      })}</tbody></table></div>}
    </section>
  </section>
}
